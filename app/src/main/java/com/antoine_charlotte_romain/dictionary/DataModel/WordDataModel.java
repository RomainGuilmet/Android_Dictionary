...
public class WordDataModel extends DAOBase{
    ...
    private static final String SQL_SELECT_WORD_WITH_BEGIN_MIDDLE_END_WHOLE_WORD = "SELECT * FROM " + WordEntry.TABLE_NAME +
            " WHERE " + WordEntry.COLUMN_NAME_HEADWORD + " LIKE ?%?%?" +
            " OR " + WordEntry.COLUMN_NAME_TRANSLATION + " LIKE ?%?%?" +
            " OR " + WordEntry.COLUMN_NAME_NOTE + " LIKE ?%?%?;";

    private static final String SQL_SELECT_WORD_WITH_BEGIN_MIDDLE_END_WHOLE_WORD_AND_DICTIONARY = "SELECT * FROM " + WordEntry.TABLE_NAME +
            " WHERE " + WordEntry.COLUMN_NAME_HEADWORD + " LIKE ?%?%?" +
            " OR " + WordEntry.COLUMN_NAME_TRANSLATION + " LIKE ?%?%?" +
            " OR " + WordEntry.COLUMN_NAME_NOTE + " LIKE ?%?%?" +
            " AND " + WordEntry.COLUMN_NAME_DICTIONARY_ID + " = ?;";
    ...
    public ArrayList<Word> selectWholeWordWithBeginMiddleEnd(String begin, String middle, String end, long dictionaryID){
        ...
        Cursor c;
        if(dictionaryID == Word.ALL_DICTIONARIES) {
            c = db.rawQuery(SQL_SELECT_WORD_WITH_BEGIN_MIDDLE_END_WHOLE_WORD, new String[]{String.valueOf(begin), String.valueOf(middle), String.valueOf(end), String.valueOf(dictionaryID)});
        }
        else{
            c = db.rawQuery(SQL_SELECT_WORD_WITH_BEGIN_MIDDLE_END_WHOLE_WORD_AND_DICTIONARY, new String[]{String.valueOf(begin), String.valueOf(middle), String.valueOf(end), String.valueOf(dictionaryID)});
        }
